...
- :func:`antpairs_to_blocks` is the trivial 1-antenna x 1-antenna block per requested
  pair -- a direct, bugfixed, renamed replacement for the stalled
  ``HERA-Team/matvis#79``'s ``get_matrix_sets``. It is functionally equivalent to
  ``CPUVectorDot``/``GPUVectorDot``'s existing per-pair loop (same FLOP count, same
  per-pair BLAS-call granularity), so it exists for API completeness and as a
  reference/fixture, not as a performance win in its own right.
- :func:`find_redundant_antpairs` finds one representative antenna pair per unique
  (rounded) baseline vector; refactored out of ``matvis.cli.get_redundancies`` so it's
...
def find_redundant_antpairs(
    ...
    Refactored out of ``matvis.cli.get_redundancies`` (which now delegates to this
    function) so it's independently testable/reusable outside the CLI, with the
    addition of an optional ``antpairs`` restriction.
