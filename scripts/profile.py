...
def run(analytic_beam, nfreq, ntimes, nants, nsource, gpu, double_precision):
    """Run the script."""
    (
        ants,
        flux,
        ...
        cpu_beams,
        hera_lat,
    ) = get_standard_sim_params(analytic_beam, nfreq, ntimes, nants, nsource)
    ...
    for thing, (hits, time, time_per_hit, percent) in thing_stats.items():
        print(
            f"{thing:>19}: {hits:>} hits, {time:.2f} seconds, {time_per_hit:.2f} sec/hit, {percent:3.2f}%"
        )
...
def get_summary_stats(profiler, gpu):
    ...
    thing_stats = {"total": (1, total_time, total_time / 1, 100)}
    for thing, lines in ids.items():
        ...
        # save (hits, time, time/hits, percent)
        thing_stats[thing] = (
            d[assoc_lines[0]][0],
...
def get_standard_sim_params(use_analytic_beam: bool, nfreq, ntime, nants, nsource):
