...
class TestGetRequiredChunks:
    ...
    def test_returns_at_least_one(self):
        """Result is always >= 1."""
        result = _utils.get_required_chunks(**self._base_kwargs)
        assert result >= 1
    ...
    def test_lower_memory_buffer_increases_chunks(self):
        ...
        chunks_high = _utils.get_required_chunks(
            **{**self._base_kwargs, "memory_buffer": 0.9}
        )
        chunks_low = _utils.get_required_chunks(
            **{**self._base_kwargs, "memory_buffer": 0.1}
        )
